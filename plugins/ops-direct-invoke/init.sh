...
setup_repo() {
    ...
    local repo_dir="${CANNBOT_MID_DIR}/dependencies/${TEAM_NAME}/${name}"
...
REPOSITORY_ROOT="$(cd "${PLUGIN_ROOT}/../.." 2>/dev/null && pwd || true)"

# 源码安装时 vendor/cannbot-skills 可能尚未初始化（普通 git clone 未拉 submodule）。
# 惰性初始化：仅在仓库根存在且 submodule 空时执行，失败不阻断（后续 resolve 会告警缺失）。
if [ -n "${REPOSITORY_ROOT}" ] && [ -d "${REPOSITORY_ROOT}/.git" ]; then
    if [ ! -d "${REPOSITORY_ROOT}/vendor/cannbot-skills/ops" ]; then
        if git -C "${REPOSITORY_ROOT}" submodule update --init --recursive --depth 1 vendor/cannbot-skills >/dev/null 2>&1; then
            info "vendor/cannbot-skills submodule initialized"
        else
            warn "vendor/cannbot-skills submodule init failed; shared skills may be missing"
        fi
    fi
fi

SHARED_SKILL_ROOT="$(cd "${PLUGIN_ROOT}/../../vendor/cannbot-skills/ops" 2>/dev/null && pwd || true)"
INFRA_SKILL_ROOT="$(cd "${PLUGIN_ROOT}/../../vendor/cannbot-skills/infra" 2>/dev/null && pwd || true)"
...
Intermediate directory:
  ...
  .cannbot/dependencies/ops-direct-invoke/asc-devkit    asc-devkit repo
  .cannbot/dependencies/ops-direct-invoke/cann-samples  cann-samples repo
  .cannbot/dependencies/ops-direct-invoke/ops-tensor    ops-tensor repo
...
for entry in "${REPO_REGISTRY[@]}"; do
    ...
    repo_dir="${CANNBOT_MID_DIR}/dependencies/${TEAM_NAME}/${name}"
...
for entry in "${REPO_REGISTRY[@]}"; do
    ...
    REPO_DIRS_JSON="${REPO_DIRS_JSON}  \"${key}\": \"${CANNBOT_MID_DIR}/dependencies/${TEAM_NAME}/${name}\",
"
done

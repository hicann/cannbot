...
#
# CANNBot:   https://gitcode.com/cann/cannbot
# This file: https://gitcode.com/cann/cannbot/blob/main/plugins/ops-direct-invoke/example/init.sh
...
# Configuration
# ============================================================
CANNBOT_URL="https://gitcode.com/cann/cannbot.git"
...
resolve_base_init() {
    if [ -n "${SKILLS_REPO:-}" ] && [ -f "${SKILLS_REPO}/plugins/${PLUGIN_NAME}/init.sh" ]; then
        echo "${SKILLS_REPO}/plugins/${PLUGIN_NAME}/init.sh"
    fi
}
...
query_supported_tools() {
    ...
    echo "(运行一次 init 或通过 --repo cannbot:<path> 指定后，此列表由基类 init 动态提供)"
}
...
Options:
  ...
  --repo <name>:<path>      Use a local repository instead of cloning.
                            Supported names:
                              cannbot  - plugin orchestration repo (consumed here)
                              asc-devkit / cann-samples / ops-tensor
                                             - passed through to the base plugin

Examples:
  ./init.sh <target> --repo cannbot:~/cannbot
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --repo)
            ...
            repo_path="${2#*:}"
            case "$repo_name" in
                cannbot|asc-devkit|cann-samples|ops-tensor) ;;
                *)
                    err "Unknown repo name '$repo_name' " \
                        "(valid: cannbot / asc-devkit / cann-samples / ops-tensor)"
                    exit 1
                    ;;
            esac
            if [ ! -d "$repo_path" ]; then
...
# Determine cannbot repository location (path only; clone happens later)
if [ -n "$CANNBOT_LOCAL_PATH" ]; then
    ...
    if [ ! -d "$CANNBOT_LOCAL_PATH" ]; then
        err "cannbot directory not found: $CANNBOT_LOCAL_PATH"
        exit 1
    fi
    SKILLS_REPO="$CANNBOT_LOCAL_PATH"
else
    SKILLS_REPO="$REPO_DIR/.cannbot/cannbot"
fi
...
# ============================================================
# Step 1: Setup cannbot
# ============================================================
step "Setting up cannbot..."
if [ -n "$CANNBOT_LOCAL_PATH" ]; then
    ok "Using local cannbot at $SKILLS_REPO"
else
    if [ -d "$SKILLS_REPO/.git" ]; then
        git -C "$SKILLS_REPO" pull --quiet 2>/dev/null || true
        ok "cannbot updated (local cache)"
    else
        mkdir -p "$(dirname "$SKILLS_REPO")"
        if git clone --quiet "$CANNBOT_URL" "$SKILLS_REPO" 2>/dev/null; then
            ok "cannbot cloned from $CANNBOT_URL"
        else
            err "Failed to clone cannbot from $CANNBOT_URL"
            exit 1
        fi
    fi
fi
...
# ============================================================
PLUGIN_INIT="$SKILLS_REPO/plugins/${PLUGIN_NAME}/init.sh"

...
- **基类 init**（`plugins/ops-direct-invoke/init.sh`）：搭建基础工作区——链接主 Agent、扁平化链接 agents、收集并链接 skills、链接权限 hook（opencode 插件 / claude settings.json 注册）、clone 依赖仓；`--override` 在同一次运行内用 `<dir>/skills/` 完成子仓 skill 覆盖。

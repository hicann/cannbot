...
```
cannbot/plugins/ops-direct-invoke/   # 基类插件根 = PLUGIN_ROOT
...
共享 skill 还可能来自插件外的 `../../../vendor/cannbot-skills/ops/`、`../../../vendor/cannbot-skills/infra/`（见下方解析顺序）。
...
├── .cannbot/              ->  中间文件 + dependencies/ops-direct-invoke/（asc-devkit + cann-samples + ops-tensor）
...
### 基类 init（`plugins/ops-direct-invoke/init.sh`）
...
| 5/6 | clone asc-devkit / cann-samples / ops-tensor 到 `.cannbot/dependencies/ops-direct-invoke/` | — |
...
**skill 源解析顺序**：本地 `skills/` → 本地 `plugin-*/` 嵌套子 skill → override `skills/` → override `plugin-*/` 嵌套子 skill → 共享 `../../../vendor/cannbot-skills/ops/` → `../../../vendor/cannbot-skills/infra/`。同名时靠前者优先。

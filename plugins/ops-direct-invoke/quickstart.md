...
bash init.sh          # 默认项目级 + opencode，装到当前目录
```
...
bash init.sh project opencode /path/to/your_project     # 装到指定目录
```
...
├── 环境信息.md                  # 环境统计
└── dependencies/ops-direct-invoke/       # 依赖仓（asc-devkit/cann-samples/ops-tensor）
```
...
```bash
bash init.sh project opencode --plugin-enable plugin-pr-submit on
```

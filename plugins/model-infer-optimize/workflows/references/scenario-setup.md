...
> 路径约定：文中 `cann-recipes-infer/...`（如 `cann-recipes-infer/executor/scripts/set_env.sh`、`cann-recipes-infer/dataset/default_prompt.json`）是相对仓库根的**文件位置**（安装器在目标项目中准备的参考仓），用于"去哪找 / 改这个文件"；`cd` 进模型目录后命令里出现的 `infer.sh`、`config/<scenario>.yaml`、`infer.py` 是相对当前模型目录的。两类路径不要拼接。
